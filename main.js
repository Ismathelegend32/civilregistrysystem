...
function initLocalDB() {
    const DATA_VERSION = '3.0'; // Updated for realistic names
    ...
    if (!localStorage.getItem('crs_users')) {
        const initialUsers = [
            { id: 1, username: 'admin', password: 'admin432', email: 'admin@example.com', full_name: 'System Administrator', role: 'admin', created_at: new Date().toISOString() },
            { id: 2, username: 'ismacil', password: 'isma123', email: 'ismacil@example.com', full_name: 'Ismail Ahmed', role: 'staff', created_at: new Date().toISOString() }
        ];
    ...
    if (toggleBtn && sidebar) {
        toggleBtn.addEventListener('click', () => {
            ...
            if (sidebar.classList.contains('collapsed')) {
                ...
                mainContent.classList.toggle('ml-20', true);
            } else {
                mainContent.classList.toggle('ml-20', false);
                mainContent.classList.toggle('ml-64', true);
            }
        });
    }
});
